...
static int rk_display_init(struct udevice *dev, ulong fbbase, ofnode vp_node)
{
	...
	if (bridge) {
		...
		/* Get panel rotation value and divide it by 90 */
		panel_rot = dev_read_u32_default(panel, "rotation", 0);
		if (panel_rot > 0) {
			panel_rot = (panel_rot / 90) % 4;
			debug("%s(%s): panel rotation %d\n", __func__, dev_read_name(dev),
				panel_rot);
		}
	...
	uc_priv->rot = panel_rot;
	uc_priv->bpix = l2bpp;

	debug("fb=%lx, rot=%d, size=%d %d\n", fbbase, uc_priv->rot,
